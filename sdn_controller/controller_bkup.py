...
class SimpleMonitor13(app_manager.RyuApp):
    ...
    def _monitor(self):
        while True:
            ...
            # 1. Mở khóa IP hết hạn
            expired_ips = [ip for ip, data in self.blocked_ips.items() if current_ts > data['unlock_time']]
            for ip in expired_ips: del self.blocked_ips[ip]

            # 2. Reset khóa hiển thị dashboard
            if time.time() >= self.pred_lock_until:
                self.pred_lock_priority = -1
                self.latest_pred['priority'] = -1
                    
            # 3. Gửi Request lấy thống kê
            for dp in self.datapaths.values():
                self._request_stats(dp)
                self._check_manual_blocks(dp)
            
            # 4. [LOGIC MỚI] Tính tổng băng thông toàn mạng và ghi file
            total_attack_bw = sum(s.get('attack', 0) for s in self.switch_traffic.values())
            total_benign_bw = sum(s.get('benign', 0) for s in self.switch_traffic.values())
            
            try:
                timestamp_str = datetime.now().strftime('%H:%M:%S')
                with open(self.TRAFFIC_MONITOR_FILE, "a") as f:
                    f.write(f"{timestamp_str},{total_attack_bw:.4f},{total_benign_bw:.4f}\n")
            except: pass
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        # Biến tạm tính băng thông cho Switch này
        current_attack_mbps = 0.0
        current_benign_mbps = 0.0
        ...
        for stat in body:
            ...
            if stat.priority >= 100:
                ...
                proto = ip_proto
            flow_key = (dpid, src, dst, proto, stat.priority)
